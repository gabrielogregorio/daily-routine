...
export const Home = () => {
  ...
  const [taskLists, setTaskLists] =
    useState<taskListType[]>(localStorageTaskList);
  const taskListsSorted: taskListType[] = taskLists.sort((a, b) =>
    a.order > b.order ? 1 : -1
  );
  ...
  function handleReturn(event: any) {
    if (event.key === "Enter") {
      ...
      if (taskLists.length > 0) {
        const maxOrderObject = taskLists.reduce((max, obj) =>
          max.order > obj.order ? max : obj
        );
  ...
  function deleteTask(taskOrder: number) {
    const taskListsRemains: taskListType[] = taskLists.filter(
      (task) => task.order !== taskOrder
    );
    ...
    <>
      <main className={styles.main}>
        {taskListsSorted.map(({ order, name }, index) => {
          return (
            <div key={order} className={styles.task}>
              <button className={styles.task__name}>
                <span>{name}</span>
              </button>

              <div className={styles.task__arrow}>
                <button onClick={() => putPosition("TOP", index)}>
                  <span>arrow-top</span>
                  <BiUpArrow />
                </button>
                <button onClick={() => putPosition("BOTTOM", index)}>
                  <span>arrow-bottom</span>
                  <BiDownArrow />
                </button>
              </div>
              <button
                className={styles.task__close}
                onClick={() => deleteTask(order)}
              >
                X
              </button>
            </div>
